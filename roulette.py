...
while p1.health > 0 and dealer.health > 0:
    ...
    # Start turns
    turn = True  # User gets the first chance

    while sg.rounds and p1.health > 0 and dealer.health > 0:
        ...
        if (turn and (not p1.turnsWaiting)) or dealer.turnsWaiting:
            ...
            while sg.rounds:
                ...
                if opt == "a":  # ======== > PLAYER USE ITEM
                    inp = ""
                    while (not inp.isdigit() or int(inp) < 1 or int(inp) > len(p1.items)) and inp not in [
                        "quit", "q", "back", "x"]:
                        inp = input("Use: ")
